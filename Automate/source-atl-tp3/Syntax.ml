...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | (EOSToken | RightParenthesisToken | ElseToken | ThenToken) ->
    (print_endline "");
    (Success stream)
    | _ -> Failure)
    ...
    | MinusToken ->
      ...
      | (Success next1) ->
        (match (parseT next1) with
          ...
          | _ -> Failure)
      | _ -> Failure) 
    (* regle #11 - TX ->          | { $, =, ), else, then } *)
    | ElseToken | ThenToken | EOSToken | EqualToken | RightParenthesisToken -> (print_endline "");(Success stream)
    | _ -> Failure)
    ...
    | DivideToken ->
      ...
      | (Success next1) ->
        (match (parseF next1) with
          ...
          | _ -> Failure)
      | _ -> Failure)
    (* regle #15 - FX ->          | { $, +, -, =, ), else, then } *)
    | ElseToken | ThenToken | EOSToken | PlusToken | MinusToken | EqualToken | RightParenthesisToken -> (print_endline "");(Success stream)
    | _ -> Failure)
