...
Evalue un polynome en une valeur donnée
x : la valeur donnée
poly : un polynome codé avec une float list
Retour : équivalent à poly sans zéro superflu
*)
let rec evalue x poly = 
  match poly with
  | [] -> 0.0
  | hd::tl -> hd +. x *.(evalue x tl)

let%test _ = evalue 1. p0 = 2.5

let%test _ = evalue 0. p1 = 2.
let%test _ = evalue 1. p1 = 5.
let%test _ = evalue 100. p1 = 302.

let%test _ = evalue 0. p2 = 0.
let%test _ = evalue 1. p2 = 5.5
let%test _ = evalue 100. p1 = -19498.

let%test _ = evalue 0. p3 = 4.5
let%test _ = evalue 1. p3 = -1.
let%test _ = evalue 100. p3 = -8034395.5
...
let rec retract poly = 
  ...
  | 0.::[] -> []
  | hd::[] -> hd::[]
  | hd::bd::tl -> if hd = 0. && (retract (bd::(retract tl))) = [] then [] 
                  else hd::(retract (bd::(retract tl)))
...
let plus poly1 poly2 = 
  let rec plus_rec pol1 pol2 =
  match (pol1,pol2) with
  | ([],[]) -> []
  | ([],_) -> pol2
  | (_,[]) -> pol1
  | (hd1::tl1,hd2::tl2) -> (hd1+.hd2)::(plus_rec tl1 tl2)
  in retract (plus_rec poly1 poly2)
...
let decoder code arbre = 
  let rec aux_decoder codeList arbreOrigin arbreCour =
  match (codeList,arbreCour)  with
  | (_,Vide) -> raise CodeNonValide
  | ([],Lettre lettre) -> lettre::[]
  | (_,Lettre lettre) -> lettre::(aux_decoder codeList arbreOrigin arbreOrigin)
  | ([],_) -> raise CodeNonValide
  | ((hd::tl),Noeud(arbre1,arbre2,arbre3)) -> match hd with
    | 1 -> aux_decoder tl arbreOrigin arbre1
    | 2 -> aux_decoder tl arbreOrigin arbre2
    | 3 -> aux_decoder tl arbreOrigin arbre3
    | _ -> raise CodeNonValide
  in recompose_chaine(aux_decoder (decompose_int code) arbre arbre)
  (*fun _ -> assert false*)
...
let arbre_to_liste = 
   fun _ -> [] (*
  let rec arbre_to_liste_rec a =
  match a with
  | Vide -> []
  | Lettre lettre -> [(lettre,[])]
  | Noeud(a1,a2,a3) -> (List.map (fun (lettre,code) -> (lettre,1::code)) (arbre_to_liste_rec a1))@(List.map (fun (lettre,code) -> (lettre,2::code)) (arbre_to_liste_rec a2))@(List.map (fun (lettre,code) -> (lettre,3::code)) (arbre_to_liste_rec a))
   in List.map (fun (lettre,code) -> (lettre, recompose_int code)) (arbre_to_liste_rec a) *)

  (*let rec parcoursBranches listeBranches = 
      match listeBranches with
      | [] -> []
      | (lettre,(Noeud(boolean,listeSuivantes)))::tl -> if boolean then
         [[lettre]]@(List.map (fun liste -> lettre::liste) (parcoursBranches listeSuivantes))@(parcoursBranches tl)
      else (List.map (fun liste -> lettre::liste) (parcoursBranches listeSuivantes))@(parcoursBranches tl)
   in parcoursBranches listeBranchesInit*)
