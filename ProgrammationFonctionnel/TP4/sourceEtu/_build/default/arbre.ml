...
let%test _ = appartient_arbre ['b';'a';'s';'e']  arbre_sujet
...
(******************************************************************************)
(*   fonction de retrait d'un élément d'un arbre                              *)
...
let rec retrait_arbre element (Noeud(booleen, listeBranche)) =
   match element with
   | [] -> Noeud(false,listeBranche)
   | hd::tl -> let l = recherche hd listeBranche in
      match l with
      | None   -> Noeud(booleen,listeBranche)
      | Some a -> Noeud (booleen, maj hd (retrait_arbre tl a) listeBranche)


let parcours (Noeud(_,listeBranchesInit)) =
...
let normalize arbre =
   match arbre with
   | (Noeud(boolean,[])) -> if boolean then (Noeud(boolean,[])) else []
   | (Noeud(boolean,listeSuivante)) -> (normalize Noeud(boolean,(List.map (normalize) listeSuivante))) 
  *) 