...
package body LR.Affic is
   ...
   begin
     ...
     procedure Check_Valid_State(Widget : access Gtk_Widget_Record'Class; Context: Cairo.Cairo_Context) is
        Nb_R, Nb_L : Natural := 0;
     begin
        for I in Les_Lecteurs'range loop
           if Les_Lecteurs(I).Etat = Lecteur_Lit then
              Nb_L := Nb_L + 1;
           end if;
        end loop;
        for I in Les_Redacteurs'range loop
           if Les_Redacteurs(I).Etat = Redacteur_Ecrit then
                 Nb_R := Nb_R + 1;
           end if;
        end loop;
        if ((Nb_L > 0) and (Nb_R > 0)) or (Nb_R > 1) then
           Cairo.Set_Source_RGB(Context, 1.0, 0.0 ,0.0);
           Cairo.Rectangle(Context, 0.0, 0.0, GDouble(Widget.Get_Allocated_Width), GDouble(Widget.Get_Allocated_Height));
           Cairo.Fill(Context);
        end if;
     end Check_Valid_State;
     
     function Draw_Callback_Inside(Widget : access Gtk_Widget_Record'Class; Context: Cairo.Cairo_Context) return Boolean is
     begin
        Check_Valid_State(Widget, Context);
        for I in Les_Lecteurs'range loop
           case Les_Lecteurs(I).Etat is
              when Lecteur_Lit =>
